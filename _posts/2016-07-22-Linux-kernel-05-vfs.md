...
每一个文件都有对应的inode，里面包含了与该文件有关的一些信息。另外系统为了加速查找引入了目录项dentry结构，为了实现进程中对文件的对象引入了文件对象file。
...
4. 目录项中的i节点编号只能指向同一文件系统中的i节点。所以ln(1)命令不能跨越文件系统
...
1. i-node 2549指向一个目录块，即位testdir目录的实际数据块
2. i-node 2549的引用计数位2，任何叶子目录的引用计数都是2.一个来自父目录的引用，一个来自自身目录的dot
...
索引节点和超级块一样时实际存储在磁盘上的，当被进程访问到时才会在内存中创建
...
存放目录项与对应文件进行链接的信息，并不实际存放在磁盘上。

由于索引节点对象的属性非常多，在查找、比较文件时效率不高，所以引入了目录项的概念。

路径中的每个部分都是一个目录项，比如路径：/mnt/cdrom/foo/bar/test.txt 其中包含6个目录项，/ mnt cdrom foo bar test.txt
...
文件对象反过来指向一个目录项对象，目录项对象再指向一个索引节点
